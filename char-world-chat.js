...
createApp({
  setup() {
    ...
    const toggleNovelAware = (novel) => {
      const settings = privateChatSettingMode.value === 'other' ? otherNovelAwareSettings.value : novelAwareSettings.value;
      if (settings[novel.id]) {
        delete settings[novel.id];
    ...
    const getNovelSetting = (novelId) => {
      const settings = privateChatSettingMode.value === 'other' ? otherNovelAwareSettings.value : novelAwareSettings.value;
      return settings[novelId] || null;
    };

    const toggleChapterItem = (novelId, field, chapterIndex) => {
      const settings = privateChatSettingMode.value === 'other' ? otherNovelAwareSettings.value : novelAwareSettings.value;
      if (!settings[novelId]) return;
      const arr = settings[novelId][field];
    ...
    const charId = parseInt(params.get('charId') || params.get('id'));
    ...
    // ===== 私聊设置模式 =====
    const privateChatSettingMode = ref(''); // '' | 'other' | 'self'
    const otherCharId = ref(null);
    const otherCharNameInput = ref('');
    const otherCharWorldInput = ref('');
    const otherCharPersonaInput = ref('');
    const otherCharRealNameInput = ref('');
    const otherAiReadCountInput = ref(20);
    const otherRealtimeTimeOn = ref(false);
    const otherSelectedWorldBooks = ref([]);
    const otherHotAwareOn = ref(false);
    const otherHotAwarePlatforms = ref([]);
    const otherHotAwareCounts = ref({});
    const otherNovelAwareOn = ref(false);
    const otherNovelAwareSettings = ref({});
    const otherForeignOn = ref(false);
    const otherForeignLang = ref('日语');
    const otherForeignLangCustom = ref('');
    const otherTranslateOn = ref(false);
    const otherTranslateLang = ref('zh-CN');
    const privateChatRelation = ref('');
...
const applySummaryPromptPreset = (p) => {
  summaryPrompt.value = p.prompt;
    ...
    const toggleWorldBook = (id) => {
      const arr = privateChatSettingMode.value === 'other' ? otherSelectedWorldBooks.value : selectedWorldBooks.value;
    ...
    const selectAllCat = (cat) => {
      ...
      if (allSelected) {
        const newArr = arr.filter(id => !ids.includes(id));
        if (privateChatSettingMode.value === 'other') otherSelectedWorldBooks.value = newArr;
        else selectedWorldBooks.value = newArr;
      } else {
    ...
    const cwMode = params.get('mode') || 'my';
    const cwcIsRight = (msg) => {
      if (cwMode === 'my') {
        return msg.role === 'char';
      } else if (cwMode === 'private') {
        // 私聊模式：当前角色（charId对应的角色）的消息在右边，对方在左边
        // senderName 等于当前角色名的在右边，contact角色在左边，user在左边
        if (msg.role === 'user') return false;
        if (msg.role === 'contact') return false;
        if (msg.senderName && msg.senderName !== charName.value) return false;
        return true;
      } else if (cwMode === 'room') {
        return msg.role === 'char' && (msg.senderName === charName.value || !msg.senderName);
      }
      return msg.role === 'char';
    };


    // 获取左侧头像URL（我的消息或其他群成员消息）
    const getLeftAvatarUrl = (msg) => {
      if (msg.role === 'user') return myAvatar.value;
      // private模式：对方角色头像
      if (cwMode === 'private' && msg.role === 'contact') {
        const beauty = charContacts.value.find(c => c.name === msg.senderName);
        return beauty?.avatar || '';
      }
      // room模式：其他成员头像
      if (cwMode === 'room' && msg.role === 'other') {
        const contact = charContacts.value.find(c => c.name === msg.senderName);
        return contact?.avatar || '';
      }
      return '';
    };

    const getLeftAvatarStyle = (msg) => {
      const url = getLeftAvatarUrl(msg);
      return url ? `background-image:url(${url})` : '';
    };

    const getLeftAvatarInitial = (msg) => {
      if (msg.role === 'user') return myName.value[0] || '我';
      if (msg.role === 'contact' || msg.role === 'other') return (msg.senderName || '?')[0];
      return (msg.senderName || charName.value)[0] || '?';
    };

    const goBack = () => {
      const cwCharId = params.get('charId');
      if (cwCharId) { window.location.href = `char-world-list.html?charId=${cwCharId}`; }
    ...
    const sendMsg = async () => {
      ...
      if (!text) return;
      // 角色视角（my模式）下，用户发的消息以角色身份发出，标记为代发
      const isCharView = cwMode === 'my';
      const msg = {
        id: Date.now(),
        role: isCharView ? 'char' : 'user',
        content: text,
        type: 'normal',
        quoteId: quotingMsg.value ? quotingMsg.value.id : null,
        recalled: false,
        revealed: false,
        blockedByCharWhenSent: iBlockedByChar.value,
        timestamp: Date.now(),
        proxyByUser: isCharView ? true : false  // 标记为用户代发
      };
...
const runMemorySearch = async () => {
  ...
  const logParts = [];
  if (cache.coreMems?.length) logParts.push(`核心记忆${cache.coreMems.length}条`);
  if (cache.charMems?.length) {
    logParts.push(`私人记忆${cache.charMems.length}条：${cache.charMems.map(m => m.summary.slice(0,10)+'…').join('、')}`);
  }
  if (cache.beforeSummaries?.length || cache.afterSummaries?.length) {
    const total = (cache.beforeSummaries?.length || 0) + (cache.afterSummaries?.length || 0);
    logParts.push(`回忆摘要${total}条`);
  }
  addCharLog(`记忆检索完成：${logParts.length ? logParts.join('，') : '无相关记忆'}`);
};
...
function buildSpeakerOfflineCorpus(speaker = {}) {
  const role = speaker.role || 'char';
  const name = speaker.name || charName.value;
  const persona = speaker.persona || '';

  const sourceMsgs = allMessages.value
    .filter(m => {
      if (m.recalled || m.loading || m.simulated || m.type !== 'normal') return false;
      if (role === 'char') return m.role === 'char';
      if (role === 'contact') return m.role === 'contact';
      if (role === 'other') return m.role === 'other' && (m.senderName || '') === name;
      return false;
    })
    .slice(-80);
  ...
  return {
    ...
    suffixes,
    prefixes,
    style: extractPersonaStyle(persona || charPersona.value || '')
  };
}
...
function buildSpeakerBucketThemes(bucketKey, topics, corpus, speaker = {}) {
  ...
  if (speaker.role === 'contact' || speaker.role === 'other') {
    pool.push(`我这边刚忙完一点`);
    pool.push(`我刚刚才看到`);
    pool.push(`我现在才空下来`);
...
function generateBackfillBubblesForSpeaker(bucketKey, speaker = {}, recentGeneratedTexts = []) {
  ...
  const recentRealTexts = allMessages.value
    .filter(m => {
      if (m.recalled || m.loading) return false;
      if (speaker.role === 'char') return m.role === 'char';
      if (speaker.role === 'contact') return m.role === 'contact';
      if (speaker.role === 'other') return m.role === 'other' && (m.senderName || '') === (speaker.name || '');
      return false;
    })
    .slice(-12)
    .map(m => m.content);
  ...
  for (let i = 0; i < 8; i++) {
    let candidate = '';
    if (historyPool.length && Math.random() < 0.48) {
      candidate = pick(historyPool);
    } else {
      candidate = pick(themePool);
    }
...
const callApiPrivate = async () => {
  ...
  const otherName = pc.otherName;
  const contacts = (await dbGet(`cwContacts_${charId}`)) || [];
  const otherContact = contacts.find(c => c.id === pc.otherId);
  const otherPersona = otherContact?.persona || '';

  const loadingMsg = { id: Date.now(), role: 'char', content: '', type: 'normal', loading: true, recalled: false, revealed: false, senderName: otherName };
  allMessages.value.push(loadingMsg);
  nextTick(() => { scrollToBottom(); refreshIcons(); });
  // 读取两角色的私人记忆
  const memGlobalSettings2 = JSON.parse(JSON.stringify((await dbGet('memoryGlobalSettings')) || {}));
  const memInjectOn2 = memGlobalSettings2.injectOn !== false;
  const memInjectCount2 = parseInt(memGlobalSettings2.myChatsCount) || 20;
  let charMemText = '';
  let otherMemText = '';
  if (memInjectOn2) {
    const charMemData2 = JSON.parse(JSON.stringify((await dbGet(`charMemory_${charId}`)) || []));
    const charMemGroups2 = JSON.parse(JSON.stringify((await dbGet(`charMemoryGroups_${charId}`)) || []));
    const validCharMems = charMemData2.filter(m => {
      if (m.hidden) return false;
      let injectTo;
      if (m.injectOverride) { injectTo = m.injectOverride; }
      else {
        const grp = charMemGroups2.find(g => g.groupKey === m.groupKey);
        injectTo = grp ? grp.injectTo : { myChats: [charId], groups: [] };
      }
      return injectTo && injectTo.myChats && injectTo.myChats.some(id => String(id) === String(charId));
    });
    const topCharMems = validCharMems.slice().sort((a, b) => b.score - a.score).slice(0, memInjectCount2);
    if (topCharMems.length) {
      charMemText = topCharMems.map(m => `[${m.score.toFixed(2)}] ${m.summary}`).join('；');
    }

    // 对面角色的记忆
    const otherMemData = JSON.parse(JSON.stringify((await dbGet(`charMemory_${pc.otherId}`)) || []));
    const otherMemGroups = JSON.parse(JSON.stringify((await dbGet(`charMemoryGroups_${pc.otherId}`)) || []));
    const validOtherMems = otherMemData.filter(m => {
      if (m.hidden) return false;
      let injectTo;
      if (m.injectOverride) { injectTo = m.injectOverride; }
      else {
        const grp = otherMemGroups.find(g => g.groupKey === m.groupKey);
        injectTo = grp ? grp.injectTo : { myChats: [pc.otherId], groups: [] };
      }
      return injectTo && injectTo.myChats && injectTo.myChats.includes(pc.otherId);
    });
    const topOtherMems = validOtherMems.slice().sort((a, b) => b.score - a.score).slice(0, memInjectCount2);
    if (topOtherMems.length) {
      otherMemText = topOtherMems.map(m => `[${m.score.toFixed(2)}] ${m.summary}`).join('；');
    }
  }

  const recentMsgs = allMessages.value.filter(m => !m.recalled && !m.loading).slice(-20).map(m => {
    const name = m.role === 'char' ? charName.value : (m.role === 'contact' ? otherName : myName.value);
    if (m.proxyByUser) return `${m.senderName || name}：${m.content}（此消息是用户代替${charName.value}发出的）`;
    return `${m.senderName || name}：${m.content}`;
  }).join('\n');
  ...
  // 获取summaries
  const beforeHistorySummaries = summaries.value.filter(s => s.pos === 'before_history').map(s => `【回忆摘要】${s.content}`).join('；');
  const afterSystemSummaries = summaries.value.filter(s => s.pos === 'after_system').map(s => `【回忆摘要】${s.content}`).join('；');

  const timeStr = realtimeTimeOn.value
...
【内容要求】口语化，短句，像真实聊天，符合各自人设。不要替其他角色发言，不要说自己是AI。
${beforeHistorySummaries ? '【回忆摘要】' + beforeHistorySummaries : ''}
【聊天记录】
${recentMsgs || '（对话刚开始）'}`;
  ...
  try {
    ...
    const lines = reply.split('&').map(l => l.trim()).filter(l => l.length > 0);
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const sender = line.slice(0, colonIdx).trim();
      const content = line.slice(colonIdx + 1).trim();
      if (!content) continue;
      // 当前角色的消息显示在右边(role=char)，对方消息显示在左边(role=contact)
    ...
    addCharLog(`私聊API回复成功，共${lines.length}条`);
  } catch (e) {
    allMessages.value.splice(allMessages.value.indexOf(loadingMsg), 1);
    alert('连接失败：' + e.message);
    addCharLog(`私聊API失败: ${e.message}`, 'error');
  }
...
const callApiRoom = async () => {
  ...
  // 过滤掉"我"（__me__）
  const charMembers = members.filter(m => m.id !== '__me__');
  if (!charMembers.length) return;

  const memberNames = charMembers.map(m => m.name).join('、');
  const membersDesc = charMembers.map(m => `${m.name}${m.persona ? '（人设：' + m.persona + '）' : ''}`).join('、');

  const loadingMsg = { id: Date.now(), role: 'char', content: '', type: 'normal', loading: true, recalled: false, revealed: false, senderName: '...' };
  allMessages.value.push(loadingMsg);
  nextTick(() => { scrollToBottom(); refreshIcons(); });
  // 读取群成员的私人记忆
  const roomMemGlobalSettings = JSON.parse(JSON.stringify((await dbGet('memoryGlobalSettings')) || {}));
  const roomMemInjectOn = roomMemGlobalSettings.injectOn !== false;
  const roomMemInjectCount = parseInt(roomMemGlobalSettings.groupsCount) || 3;
  const roomMemberMemTexts = {};
  if (roomMemInjectOn) {
    const allCharListForRoomMem = JSON.parse(JSON.stringify((await dbGet('charList')) || []));
    for (const m of charMembers) {
      const mChar = allCharListForRoomMem.find(c => c.name === m.name);
      if (!mChar) continue;
      const mMemData = JSON.parse(JSON.stringify((await dbGet(`charMemory_${mChar.id}`)) || []));
      const mMemGroups = JSON.parse(JSON.stringify((await dbGet(`charMemoryGroups_${mChar.id}`)) || []));
      const validMems = mMemData.filter(mem => {
        if (mem.hidden) return false;
        let injectTo;
        if (mem.injectOverride) { injectTo = mem.injectOverride; }
        else {
          const grp = mMemGroups.find(g => g.groupKey === mem.groupKey);
          injectTo = grp ? grp.injectTo : { myChats: [mChar.id], groups: [] };
        }
        return injectTo && (
          (injectTo.myChats && injectTo.myChats.includes(mChar.id)) ||
          (injectTo.groups && injectTo.groups.includes(roomId))
        );
      });
      const topMems = validMems.slice().sort((a, b) => b.score - a.score).slice(0, roomMemInjectCount);
      if (topMems.length) {
        roomMemberMemTexts[m.name] = topMems.map(mem => `[${mem.score.toFixed(2)}] ${mem.summary}`).join('；');
      }
    }
  }
  const roomMemorySection = Object.keys(roomMemberMemTexts).length
    ? `\n【成员私人记忆（各自知道，其他人不知道）】\n` +
      Object.entries(roomMemberMemTexts).map(([name, text]) => `[只有${name}知道]${text}`).join('\n')
    : '';

  const recentMsgs = allMessages.value.filter(m => !m.recalled && !m.loading).slice(-20).map(m => {
    const sender = m.senderName || (m.role === 'user' ? myName.value : charName.value);
    if (m.proxyByUser) return `${sender}：${m.content}（此消息是用户代替${charName.value}发出的）`;
...
【内容要求】口语化，短句，像真实群聊，可以互相@，符合各自人设，不要替${myName.value}发言。
【重要】${charName.value}的消息正常发，其他成员的消息也正常发，区分清楚每个人的说话风格。
【聊天记录】
${recentMsgs || '（群聊刚开始）'}
...
评分标准：普通闲聊0~0.5，较重要的事0.5~0.8，重大事情/约定/情感表达0.8~1。`;
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const line = lines[i];
       // 过滤记忆摘要行，不显示为消息
      if (line.startsWith('【记忆摘要】')) continue;
      ...
      const content = line.slice(colonIdx + 1).trim();
      if (!content) continue;
      const member = charMembers.find(m => m.name === sender);
      if (!member && sender !== charName.value) continue;
      // 当前角色消息在右边，其他成员消息在左边
      const role = sender === charName.value ? 'char' : 'other';
      const newMsg = {
        id: Date.now() + i,
        role,
        senderName: sender,
    ...
    // 解析记忆摘要并写入
    const roomMemLineMatch = reply.match(/【记忆摘要】\s*(\{[\s\S]*?\})/);
    if (roomMemLineMatch) {
      try {
        const roomMemJson = JSON.parse(roomMemLineMatch[1]);
        const roomMemSummary = roomMemJson.summary || '';
        const roomMemScore = parseFloat(roomMemJson.score) || 0.5;
        if (roomMemSummary) {
          const roomMemKey = `miniGroup_${roomId}`;
          const roomMemberNames = charMembers.map(m => m.name);
          const allCharListForRoomWrite = JSON.parse(JSON.stringify((await dbGet('charList')) || []));
          for (const m of charMembers) {
            const mChar = allCharListForRoomWrite.find(c => c.name === m.name);
            if (!mChar) continue;
            const roomMemItem = {
            ...
            const key = `charMemory_${mChar.id}`;
            const existing = JSON.parse(JSON.stringify((await dbGet(key)) || []));
            existing.unshift(roomMemItem);
            if (existing.length > 100) existing.splice(100);
            await dbSet(key, existing);
          }
          addCharLog(`群聊记忆已写入：${room.name}，评分${roomMemScore}`);
        }
      } catch(e) {}
    }
    addCharLog(`群聊API回复成功，共${lines.length}条`);
  } catch (e) {

    allMessages.value.splice(allMessages.value.indexOf(loadingMsg), 1);
    alert('连接失败：' + e.message);
    addCharLog(`群聊API失败: ${e.message}`, 'error');
  }
...
alert('连接失败：' + e.message);
    ...
    const openMySettings = async () => {
  toolbarOpen.value = false;

  if (cwMode === 'private') {
    // 私聊模式：我的设置 = 当前角色（手机持有者）的设置
    ...
    const pcId2 = params.get('pcId') ? parseInt(params.get('pcId')) : null;
    const pcs = JSON.parse(JSON.stringify((await dbGet(`cwPrivateChats_${charId}`)) || []));
    const pc = pcs.find(p => String(p.id) === String(pcId2));
    if (pc) privateChatRelation.value = pc.relation || '';

    privateChatSettingMode.value = 'self';
    chatSettingsShow.value = true;
    nextTick(() => refreshIcons());
    return;
  }

  myNameInput.value = myName.value;
  myPersonaInput.value = myPersona.value;
  mySettingsShow.value = true;
  nextTick(() => refreshIcons());
};
...
const openChatSettings = async () => {
  ...
  if (cwMode === 'private') {
    ...
    privateChatRelation.value = pc.relation || '';

    const allCharList = JSON.parse(JSON.stringify((await dbGet('charList')) || []));
    const allRandomList = JSON.parse(JSON.stringify((await dbGet('randomCharList')) || []));
    const allChars = [...allCharList, ...allRandomList];
    const otherChar = allChars.find(c => c.id === pc.otherId);

    otherCharId.value = pc.otherId;
    otherCharNameInput.value = pc.otherName;
    otherCharWorldInput.value = otherChar?.world || '';
    otherCharPersonaInput.value = otherChar?.persona || '';
    const extracted = (otherChar?.persona || '').match(/(?:中文名|Chinese\s*name|名字|姓名|真名|name)\s*(?:[：:]\s*|[是为叫]\s*)([^\s，,。;\n]+)/i)?.[1] || '';
    otherCharRealNameInput.value = extracted;
    otherAiReadCountInput.value = otherChar?.aiReadCount || 20;
    otherRealtimeTimeOn.value = otherChar?.realtimeTimeOn || false;
    otherSelectedWorldBooks.value = otherChar?.selectedWorldBooks || [];

    const otherHotData = await dbGet(`hotAware_${pc.otherId}`);
    otherHotAwareOn.value = otherHotData?.on || false;
    otherHotAwarePlatforms.value = otherHotData?.platforms || [];
    otherHotAwareCounts.value = otherHotData?.counts || {};

    const otherNovelData = await dbGet(`novelAware_${pc.otherId}`);
    otherNovelAwareOn.value = otherNovelData?.on || false;
    otherNovelAwareSettings.value = otherNovelData?.settings || {};

    const otherTranslateData = await dbGet(`chatTranslate_${pc.otherId}`);
    otherTranslateOn.value = otherTranslateData?.on || false;
    otherTranslateLang.value = otherTranslateData?.lang || 'zh-CN';
    otherForeignOn.value = otherTranslateData?.foreignOn || false;
    otherForeignLang.value = otherTranslateData?.foreignLang || '日语';
    otherForeignLangCustom.value = otherTranslateData?.foreignLangCustom || '';

    privateChatSettingMode.value = 'other';
    chatSettingsShow.value = true;
    nextTick(() => refreshIcons());
    return;
  }
  ...
  if (cwMode === 'private' && privateChatSettingMode.value === 'other') {
    // 保存对面角色的设置
    if (otherCharRealNameInput.value.trim()) {
      const hasRealName = otherCharPersonaInput.value.match(/(?:名字|姓名|真名|name)[：:是为叫]?\s*[^\s，,。.]+/);
      if (!hasRealName) {
        otherCharPersonaInput.value = `真名：${otherCharRealNameInput.value.trim()}\n` + otherCharPersonaInput.value;
      }
    }
    const allCharList = JSON.parse(JSON.stringify((await dbGet('charList')) || []));
    const idx = allCharList.findIndex(c => c.id === otherCharId.value);
    if (idx !== -1) {
      allCharList[idx].name = otherCharNameInput.value;
      allCharList[idx].world = otherCharWorldInput.value;
      allCharList[idx].persona = otherCharPersonaInput.value;
      allCharList[idx].aiReadCount = parseInt(otherAiReadCountInput.value) || 20;
      allCharList[idx].realtimeTimeOn = otherRealtimeTimeOn.value;
      allCharList[idx].selectedWorldBooks = JSON.parse(JSON.stringify(otherSelectedWorldBooks.value));
      await dbSet('charList', allCharList);
    }
    await dbSet(`hotAware_${otherCharId.value}`, JSON.parse(JSON.stringify({
      on: otherHotAwareOn.value,
      platforms: otherHotAwarePlatforms.value,
      counts: otherHotAwareCounts.value
    })));
    await dbSet(`novelAware_${otherCharId.value}`, JSON.parse(JSON.stringify({
      on: otherNovelAwareOn.value,
      settings: otherNovelAwareSettings.value
    })));
    await dbSet(`chatTranslate_${otherCharId.value}`, {
      on: otherForeignOn.value ? false : otherTranslateOn.value,
      lang: otherTranslateLang.value,
      foreignOn: otherForeignOn.value,
      foreignLang: otherForeignLang.value,
      foreignLangCustom: otherForeignLangCustom.value
    });
    // 同步私聊记录里的对方名字和关系
    const pcId2 = params.get('pcId') ? parseInt(params.get('pcId')) : null;
    if (pcId2) {
      const pcs = JSON.parse(JSON.stringify((await dbGet(`cwPrivateChats_${charId}`)) || []));
      const pcIdx = pcs.findIndex(p => String(p.id) === String(pcId2));
      if (pcIdx !== -1) {
        pcs[pcIdx].otherName = otherCharNameInput.value;
        pcs[pcIdx].relation = privateChatRelation.value;
  ...
  if (cwMode === 'private' && privateChatSettingMode.value === 'self') {
    // 保存当前角色的设置
    ...
    const allCharList = JSON.parse(JSON.stringify((await dbGet('charList')) || []));
    const idx = allCharList.findIndex(c => c.id === charId);
    if (idx !== -1) {
      allCharList[idx].name = charName.value;
      allCharList[idx].world = charWorld.value;
      allCharList[idx].persona = charPersona.value;
      allCharList[idx].aiReadCount = aiReadCount.value;
      allCharList[idx].realtimeTimeOn = realtimeTimeOn.value;
      allCharList[idx].selectedWorldBooks = JSON.parse(JSON.stringify(selectedWorldBooks.value));
      await dbSet('charList', allCharList);
    }
  ...
  if (idx !== -1) {
   ...
   const sendSticker = async (s) => {
    ...
    const applyWallpaperToDom = () => {
      const el = document.getElementById('cwc-app');
  ...
  await saveMessages();
    ...
    const messagesWithTime = computed(() => {
      ...
      const cwMode3 = params.get('mode') || 'my';
      const pcId2 = params.get('pcId') ? parseInt(params.get('pcId')) : null;
      const roomId2 = params.get('roomId') ? parseInt(params.get('roomId')) : null;
      if (cwMode3 === 'my') {
        const charList = JSON.parse(JSON.stringify((await dbGet('charList')) || []));
        const idx = charList.findIndex(c => c.id === charId);
        if (idx !== -1) { charList[idx].messages = JSON.parse(JSON.stringify(allMessages.value.filter(m => !m.loading))); charList[idx].lastMsg = allMessages.value.filter(m => !m.loading && !m.recalled).slice(-1)[0]?.content || ''; charList[idx].lastTime = Date.now(); await dbSet('charList', charList); }
        ...
        if (rIdx !== -1) { randomCharList[rIdx].messages = JSON.parse(JSON.stringify(allMessages.value.filter(m => !m.loading))); randomCharList[rIdx].lastMsg = allMessages.value.filter(m => !m.loading && !m.recalled).slice(-1)[0]?.content || ''; randomCharList[rIdx].lastTime = Date.now(); await dbSet('randomCharList', randomCharList); }
      } else if (cwMode3 === 'private' && pcId2) {
        const pcs = JSON.parse(JSON.stringify((await dbGet(`cwPrivateChats_${charId}`)) || []));
        const idx = pcs.findIndex(p => String(p.id) === String(pcId2));
        if (idx !== -1) {
          pcs[idx].messages = JSON.parse(JSON.stringify(allMessages.value.filter(m => !m.loading)));
          pcs[idx].lastMsg = allMessages.value.filter(m => !m.loading && !m.recalled).slice(-1)[0]?.content || '';
          pcs[idx].lastTime = Date.now();
          await dbSet(`cwPrivateChats_${charId}`, pcs);
          // 同步消息到对方的私聊记录
          const otherId = pcs[idx].otherId;
          if (otherId) {
            const otherPcs = JSON.parse(JSON.stringify((await dbGet(`cwPrivateChats_${otherId}`)) || []));
            const otherIdx = otherPcs.findIndex(p => p.otherId === charId);
            if (otherIdx !== -1) {
              otherPcs[otherIdx].messages = JSON.parse(JSON.stringify(allMessages.value.filter(m => !m.loading).map(m => {
                if (m.role === 'char' && m.senderName === charName.value) {
              ...
              otherPcs[otherIdx].lastMsg = pcs[idx].lastMsg;
              otherPcs[otherIdx].lastTime = Date.now();
              await dbSet(`cwPrivateChats_${otherId}`, otherPcs);
            }
          }
        }
      } else if (cwMode3 === 'room' && roomId2) {
        const roomList = JSON.parse(JSON.stringify((await dbGet('roomList')) || []));
        const idx = roomList.findIndex(r => String(r.id) === String(roomId2));
        if (idx !== -1) {
          roomList[idx].messages = JSON.parse(JSON.stringify(allMessages.value.filter(m => !m.loading)));
          roomList[idx].lastMsg = allMessages.value.filter(m => !m.loading && !m.recalled).slice(-1)[0]?.content || '';
          roomList[idx].lastTime = Date.now();
          await dbSet('roomList', roomList);
        } else {
          const localGroups = JSON.parse(JSON.stringify((await dbGet(`cwLocalGroups_${charId}`)) || []));
          const lgIdx = localGroups.findIndex(r => String(r.id) === String(roomId2));
          if (lgIdx !== -1) {
            localGroups[lgIdx].messages = JSON.parse(JSON.stringify(allMessages.value.filter(m => !m.loading)));
            localGroups[lgIdx].lastMsg = allMessages.value.filter(m => !m.loading && !m.recalled).slice(-1)[0]?.content || '';
            localGroups[lgIdx].lastTime = Date.now();
...
const confirmDeleteChar = async () => {
  ...
  if (cwMode3 === 'my') {
    ...
    const charList = JSON.parse(JSON.stringify((await dbGet('charList')) || []));
    const idx = charList.findIndex(c => c.id === charId);
    if (idx !== -1) { charList[idx].messages = []; charList[idx].lastMsg = ''; await dbSet('charList', charList); }
    const randomCharList = JSON.parse(JSON.stringify((await dbGet('randomCharList')) || []));
    const rIdx = randomCharList.findIndex(c => c.id === charId);
    if (rIdx !== -1) { randomCharList[rIdx].messages = []; randomCharList[rIdx].lastMsg = ''; await dbSet('randomCharList', randomCharList); }
    allMessages.value = [];
    window.location.href = `char-world-list.html?charId=${charId}`;
  } else if (cwMode3 === 'private' && pcId2) {
    // private模式：删除这条私聊记录
    const pcs = JSON.parse(JSON.stringify((await dbGet(`cwPrivateChats_${charId}`)) || []));
    const idx = pcs.findIndex(p => String(p.id) === String(pcId2));
    if (idx !== -1) { pcs.splice(idx, 1); await dbSet(`cwPrivateChats_${charId}`, pcs); }
    window.location.href = `char-world-list.html?charId=${charId}`;
  } else if (cwMode3 === 'room' && roomId2) {
    // room模式：清空这个群的消息
    const roomList = JSON.parse(JSON.stringify((await dbGet('roomList')) || []));
    const idx = roomList.findIndex(r => String(r.id) === String(roomId2));
    if (idx !== -1) { roomList[idx].messages = []; roomList[idx].lastMsg = ''; await dbSet('roomList', roomList); }
    const localGroups = JSON.parse(JSON.stringify((await dbGet(`cwLocalGroups_${charId}`)) || []));
    const lgIdx = localGroups.findIndex(r => String(r.id) === String(roomId2));
    if (lgIdx !== -1) { localGroups[lgIdx].messages = []; localGroups[lgIdx].lastMsg = ''; await dbSet(`cwLocalGroups_${charId}`, localGroups); }
    window.location.href = `char-world-list.html?charId=${charId}`;
  }
};
...
if (keepAliveData) {
  ...
  dbGet('randomCharList')
      ...
      if (char) {
        ...
        charWorld.value = char.world || '';
        charPersona.value = char.persona || '';
        aiReadCount.value = char.aiReadCount || 20;
        aiReadCountInput.value = char.aiReadCount || 20;
        isBlocked.value = char.isBlocked || false;
        iBlockedByChar.value = char.iBlockedByChar || false;
        realtimeTimeOn.value = char.realtimeTimeOn || false;
        const cwMode2 = params.get('mode') || 'my';
        const pcId = params.get('pcId') ? parseInt(params.get('pcId')) : null;
        const roomId = params.get('roomId') ? parseInt(params.get('roomId')) : null;
        if (cwMode2 === 'my') {
          allMessages.value = (char.messages || []).filter(m => !m.loading);
        } else if (cwMode2 === 'private' && pcId) {
          const pcs = await dbGet(`cwPrivateChats_${charId}`);
          const pc = (pcs || []).find(p => String(p.id) === String(pcId));
          if (pc) allMessages.value = (pc.messages || []).filter(m => !m.loading);
        } else if (cwMode2 === 'room' && roomId) {
          const roomList = await dbGet('roomList');
          const localGroups = await dbGet(`cwLocalGroups_${charId}`);
          const allRooms = [...(roomList || []), ...(localGroups || [])];
          const room = allRooms.find(r => String(r.id) === String(roomId));
          if (room) allMessages.value = (room.messages || []).filter(m => !m.loading);
        }
      }
      // 加载联系人列表（群聊头像需要用到）
...
if (savedMemorySearchOn !== null && savedMemorySearchOn !== undefined) {
  memorySearchOn.value = savedMemorySearchOn;
    ...
    // 当前设置模式下的小说设置（三态）
    const activeNovelAwareSettings = computed({
      get: () => privateChatSettingMode.value === 'other' ? otherNovelAwareSettings.value : novelAwareSettings.value,
      set: (val) => { if (privateChatSettingMode.value === 'other') otherNovelAwareSettings.value = val; else novelAwareSettings.value = val; }
    });
    const activeNovelAwareOn = computed({
      get: () => privateChatSettingMode.value === 'other' ? otherNovelAwareOn.value : novelAwareOn.value,
      set: (val) => { if (privateChatSettingMode.value === 'other') otherNovelAwareOn.value = val; else novelAwareOn.value = val; }
    });
...
charWorldConfirmShow, charWorldHint, charWorldShowHint, charWorldHintConfirmShow, aiSetCharWorldHint,
      cwcIsRight, getLeftAvatarUrl, getLeftAvatarStyle, getLeftAvatarInitial, charContacts, cwMode, 
      privateChatSettingMode, privateChatRelation,
      otherCharId, otherCharNameInput, otherCharWorldInput, otherCharPersonaInput, otherCharRealNameInput,
      otherAiReadCountInput, otherRealtimeTimeOn, otherSelectedWorldBooks,
      otherHotAwareOn, otherHotAwarePlatforms, otherHotAwareCounts,
      otherNovelAwareOn, otherNovelAwareSettings,
      otherForeignOn, otherForeignLang, otherForeignLangCustom,
      otherTranslateOn, otherTranslateLang,
      activeNovelAwareSettings, activeNovelAwareOn,
